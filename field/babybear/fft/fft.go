...
func kerDIFNP_256(a []babybear.Element, twiddles [][]babybear.Element, stage int) {
	...
	for offset := 0; offset < 256; offset += 8 {
		babybear.Butterfly(&a[offset], &a[offset+4])
		babybear.Butterfly(&a[offset+1], &a[offset+5])
		a[offset+5].Mul(&a[offset+5], &twiddles[stage+5][1])
		babybear.Butterfly(&a[offset+2], &a[offset+6])
		a[offset+6].Mul(&a[offset+6], &twiddles[stage+5][2])
		babybear.Butterfly(&a[offset+3], &a[offset+7])
